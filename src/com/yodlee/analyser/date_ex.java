...
public class date_ex  extends BarRenderer{
	...
	public static void generate(Map<String,HashMap<String,StringBuilder>> final_map,String t,HashMap<String,StringBuilder> timekey,TreeSet<String> tset){
			...
			if(temp.contains("java.lang.Thread.State: WAITING")){
			    if(flag==0){
			    	...
			    	if(flag!=3){
			    		...
			    		stack_trace.put(count_st,toolTipStackTrace(timekey,start,end,diff));
			    		count_st++;
			    		...
			    		if(any_lag!=0){
			    			dataset.addValue(0.5D,new UniqueComparable("UNKNOWN"),t);
		...
		//long any_lag=date_diff(end,tt);
		//start=end=tt;
		else dataset.addValue((double)(diff/(1000)),new UniqueComparable(flag_val.get(flag)),t);
			
		stack_trace.put(count_st,toolTipStackTrace(timekey,start,end,diff));
		count_st++;
		
	}
	private static String toolTipStackTrace(HashMap<String,StringBuilder> timekey,String start,String end,long diff){
		StringBuilder str=new StringBuilder();
		String source=timekey.get(start).toString();
		str.append("<html>START TIME: "+start+"<br>");
		str.append("<html>END TIME: "+end+"<br>");
		if(diff!=0)
		str.append("<html>DURATION: "+diff/1000+" seconds<br>");
		else str.append("<html>DURATION: "+1+" second<br>");
